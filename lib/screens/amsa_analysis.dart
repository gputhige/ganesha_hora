...
class AmsaAnalysis extends StatefulWidget {
  ...
  @override
  State<AmsaAnalysis> createState() => _AmsaAnalysisState();
}

class _AmsaAnalysisState extends State<AmsaAnalysis> {
  ...
  Future<int> _getGrahaDetails() async {
    ...
    for (int i = 0; i < 9; i++) {
      if (plposint - ascposint < 0) {
        plposint = _reduceRasi((plposint + 12));
      }
      _updateMap(grahaDetails, "grh", dasagraha[i].name!);
      _updateMap(grahaDetails, "deg", (planetPos[i].toStringAsFixed(2)));
      if (planetSpeed[i] < 0) {
        if (i == 7 || i == 8) {
          _updateMap(grahaDetails, 'spd', '-');
        } else {
          _updateMap(grahaDetails, 'spd', 'R');
        }
      } else {
        _updateMap(grahaDetails, 'spd', '-');
      }
      _updateMap(grahaDetails, "ras",
          (_reduceRasi(planetPos[i] ~/ 30.ceil() + 1).toString()));

      _updateMap(grahaDetails, 'rnm',
          raasi[(_reduceRasi(planetPos[i] ~/ 30.ceil() + 1)) - 1].shortname!);
      _updateMap(grahaDetails, 'rld',
          raasi[(_reduceRasi(planetPos[i] ~/ 30.ceil() + 1)) - 1].lord!);
      for (int k = 0; k < 9; k++) {
      ...
      if (plposint - ascposint < 0) {
        ...
        plposint = plposint - ascposint;
      }
      _updateMap(grahaDetails, "bhv", (plposint).toString());
...
//Calculate Exhalted,Debilitated,Moola,Own Houses and Points
  Future<int> _getGrahaDignity() async {
    ...
    for (int z = 0; z < 9; z++) {
      if (double.parse(grahaDetails['deg']![z]) >= dign[z][grahaList[z]]![0] &&
                              ...
                              for (int i = 0; i < dasagraha.length; i++)
                                Row(
                                  ...
                                  children: [
                                    ...
                                    alignedText(20, grahaDetails['spd']![i], 0),
                                    alignedText(
                                    ...
                                    alignedText(
                                        37, '${grahaDetails['dgn']![i]}', 0),
                                    alignedText(23, grahaDetails['bhv']![i], 0),
                                    /*alignedText(
